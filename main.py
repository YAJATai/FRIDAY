...
class JarvisLive:
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            try:
                self._mic_raw = float(_pcm_level(indata))
            except Exception:
                pass
            # ── Wake-word gate ───────────────────────────────────────────────
        ...
        try:
            ...
            with _mic_stream:
                ...
                while True:
                    ...
                    _lvl_log += 1
                    if _lvl_log % 30 == 0:
                        _raw = getattr(self, '_mic_raw', 0.0)
                        print(f"[FRIDAY] 🎙 mic raw = {_raw:.4f}  (gated rms = {getattr(self, '_mic_rms', 0.0):.4f})")
        except Exception as e:
            ...
            try:
                ...
                async with (
                    ...
                    if self._wake_enabled:
                        ...
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: FRIDAY online — sleeping. Say 'Hey Jarvis' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
...
def main():
    try:
        _log_dir = Path.home() / "FRIDAY" / "logs"
        _log_dir.mkdir(parents=True, exist_ok=True)
        _logf = open(_log_dir / "friday.log", "a", buffering=1)
        sys.stdout = _logf
        sys.stderr = _logf
    except Exception:
        pass
